// extension/src/shared/catalog_scores.js
// Ratings hardcodeados desde frontend/src/data/catalogServices.js.
// La extensión usa este archivo para mostrar el mismo score numérico y letra A-E
// que aparecen en la ficha editorial de GitHub Pages.

export const CATALOG_SCORE_SERVICES = [
  {
    "domain": "google.com",
    "name": "Google",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "youtube.com",
    "name": "YouTube",
    "rating": "E",
    "score": 36
  },
  {
    "domain": "facebook.com",
    "name": "Facebook",
    "rating": "E",
    "score": 36
  },
  {
    "domain": "instagram.com",
    "name": "Instagram",
    "rating": "E",
    "score": 36
  },
  {
    "domain": "apple.com",
    "name": "Apple",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "microsoft.com",
    "name": "Microsoft",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "amazon.com",
    "name": "Amazon",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "whatsapp.net",
    "name": "WhatsApp",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "netflix.com",
    "name": "Netflix",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "office.com",
    "name": "Microsoft Office",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "spotify.com",
    "name": "Spotify",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "bing.com",
    "name": "Bing",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "cloudflare.com",
    "name": "Cloudflare",
    "rating": "B",
    "score": 82
  },
  {
    "domain": "snapchat.com",
    "name": "Snapchat",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "yahoo.com",
    "name": "Yahoo",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "twitter.com",
    "name": "X",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "linkedin.com",
    "name": "LinkedIn",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "gmail.com",
    "name": "Gmail",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "samsung.com",
    "name": "Samsung",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "baidu.com",
    "name": "Baidu",
    "rating": "D",
    "score": 51
  },
  {
    "domain": "epicgames.com",
    "name": "Epic Games",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "outlook.com",
    "name": "Outlook",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "dropbox.com",
    "name": "Dropbox",
    "rating": "B",
    "score": 81
  },
  {
    "domain": "grammarly.com",
    "name": "Grammarly",
    "rating": "B",
    "score": 81
  },
  {
    "domain": "android.com",
    "name": "Android",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "qq.com",
    "name": "QQ",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "kaspersky.com",
    "name": "Kaspersky",
    "rating": "B",
    "score": 81
  },
  {
    "domain": "opera.com",
    "name": "Opera",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "avast.com",
    "name": "Avast",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "espn.com",
    "name": "ESPN",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "adobe.com",
    "name": "Adobe",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "canva.com",
    "name": "Canva",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "slack.com",
    "name": "Slack",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "nvidia.com",
    "name": "NVIDIA",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "discord.com",
    "name": "Discord",
    "rating": "D",
    "score": 50
  },
  {
    "domain": "mozilla.com",
    "name": "Mozilla",
    "rating": "B",
    "score": 80
  },
  {
    "domain": "steampowered.com",
    "name": "Steam",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "reddit.com",
    "name": "Reddit",
    "rating": "D",
    "score": 50
  },
  {
    "domain": "brave.com",
    "name": "Brave",
    "rating": "A",
    "score": 92
  },
  {
    "domain": "synology.com",
    "name": "Synology",
    "rating": "B",
    "score": 80
  },
  {
    "domain": "xiaomi.com",
    "name": "Xiaomi",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "mercadolibre.com",
    "name": "Mercado Libre",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "twitch.tv",
    "name": "Twitch",
    "rating": "D",
    "score": 50
  },
  {
    "domain": "stripe.com",
    "name": "Stripe",
    "rating": "B",
    "score": 79
  },
  {
    "domain": "hulu.com",
    "name": "Hulu",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "tiktok.com",
    "name": "TikTok",
    "rating": "E",
    "score": 33
  },
  {
    "domain": "pinterest.com",
    "name": "Pinterest",
    "rating": "D",
    "score": 49
  },
  {
    "domain": "tradingview.com",
    "name": "TradingView",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "weather.com",
    "name": "Weather.com",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "wikipedia.org",
    "name": "Wikipedia",
    "rating": "B",
    "score": 78
  },
  {
    "domain": "telegram.org",
    "name": "Telegram",
    "rating": "B",
    "score": 78
  },
  {
    "domain": "signal.org",
    "name": "Signal",
    "rating": "A",
    "score": 89
  },
  {
    "domain": "wechat.com",
    "name": "WeChat",
    "rating": "D",
    "score": 47
  },
  {
    "domain": "openai.com",
    "name": "OpenAI",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "playstation.net",
    "name": "PlayStation",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "roblox.com",
    "name": "Roblox",
    "rating": "D",
    "score": 52
  },
  {
    "domain": "ubuntu.com",
    "name": "Ubuntu",
    "rating": "B",
    "score": 80
  },
  {
    "domain": "intuit.com",
    "name": "Intuit",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "pluto.tv",
    "name": "Pluto TV",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "aliexpress.com",
    "name": "AliExpress",
    "rating": "E",
    "score": 31
  },
  {
    "domain": "taobao.com",
    "name": "Taobao",
    "rating": "D",
    "score": 47
  },
  {
    "domain": "shopee.com.br",
    "name": "Shopee",
    "rating": "D",
    "score": 47
  },
  {
    "domain": "instacart.com",
    "name": "Instacart",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "trendyol.com",
    "name": "Trendyol",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "wildberries.ru",
    "name": "Wildberries",
    "rating": "D",
    "score": 49
  },
  {
    "domain": "qantas.com",
    "name": "Qantas",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "zoom.us",
    "name": "Zoom",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "surfshark.com",
    "name": "Surfshark",
    "rating": "B",
    "score": 81
  },
  {
    "domain": "mcafee.com",
    "name": "McAfee",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "norton.com",
    "name": "Norton",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "trendmicro.com",
    "name": "Trend Micro",
    "rating": "B",
    "score": 79
  },
  {
    "domain": "bitdefender.net",
    "name": "Bitdefender",
    "rating": "B",
    "score": 78
  },
  {
    "domain": "starlink.com",
    "name": "Starlink",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "ring.com",
    "name": "Ring",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "sonos.com",
    "name": "Sonos",
    "rating": "B",
    "score": 78
  },
  {
    "domain": "ui.com",
    "name": "UI",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "icloud.com",
    "name": "Icloud",
    "rating": "B",
    "score": 82
  },
  {
    "domain": "live.com",
    "name": "Microsoft Live",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "one.one",
    "name": "1.1.1.1",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "azure.com",
    "name": "Azure",
    "rating": "B",
    "score": 82
  },
  {
    "domain": "msftncsi.com",
    "name": "Msftncsi",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "msn.com",
    "name": "MSN",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "skype.com",
    "name": "Skype",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "akamaized.net",
    "name": "Akamaized",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "sharepoint.com",
    "name": "Sharepoint",
    "rating": "C",
    "score": 67
  },
  {
    "domain": "windows.com",
    "name": "Windows",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "a-msedge.net",
    "name": "A Msedge",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "amazonalexa.com",
    "name": "Amazonalexa",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "samsungcloud.com",
    "name": "Samsung Cloud",
    "rating": "B",
    "score": 81
  },
  {
    "domain": "appcenter.ms",
    "name": "Appcenter",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "v-mate.mobi",
    "name": "VMate",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "pubgmobile.com",
    "name": "PUBG Mobile",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "helpshift.com",
    "name": "Helpshift",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "likee.video",
    "name": "Likee",
    "rating": "D",
    "score": 51
  },
  {
    "domain": "like.video",
    "name": "LIKE Video",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "bigolive.tv",
    "name": "BIGO Live",
    "rating": "D",
    "score": 51
  },
  {
    "domain": "samsungapps.com",
    "name": "Galaxy Store",
    "rating": "C",
    "score": 66
  },
  {
    "domain": "msedge.net",
    "name": "Msedge",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "anydesk.com",
    "name": "Anydesk",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "3lift.com",
    "name": "3lift",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "fast.com",
    "name": "Fast",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "yle.fi",
    "name": "Yle",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "gamepass.com",
    "name": "Gamepass",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "azurewebsites.net",
    "name": "Azurewebsites",
    "rating": "B",
    "score": 80
  },
  {
    "domain": "virtualearth.net",
    "name": "Bing Maps",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "viber.com",
    "name": "Viber",
    "rating": "C",
    "score": 65
  },
  {
    "domain": "yandex.net",
    "name": "Yandex",
    "rating": "D",
    "score": 50
  },
  {
    "domain": "vk.com",
    "name": "VK",
    "rating": "D",
    "score": 49
  },
  {
    "domain": "ok.ru",
    "name": "OK",
    "rating": "D",
    "score": 49
  },
  {
    "domain": "wb.ru",
    "name": "WB",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "miui.com",
    "name": "Miui",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "sonnenbatterie.de",
    "name": "Sonnenbatterie",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "speedtest.net",
    "name": "Speedtest",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "ssl-images-amazon.com",
    "name": "SSL Images Amazon",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "gateio.live",
    "name": "Gateio",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "smartappnet.net",
    "name": "Smartappnet",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "leroymerlin.pl",
    "name": "Leroymerlin",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "newark.com",
    "name": "Newark",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "expedia.com.au",
    "name": "Expedia",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "vrt.be",
    "name": "VRT",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "palmpay.app",
    "name": "Palmpay",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "studiotv.net",
    "name": "Studiotv",
    "rating": "C",
    "score": 64
  },
  {
    "domain": "vivoglobal.com",
    "name": "Vivoglobal",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "freefiremobile.com",
    "name": "Freefiremobile",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "heytapmobile.com",
    "name": "Heytapmobile",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "inmobi.com",
    "name": "Inmobi",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "xzcs3zlph.com",
    "name": "Xzcs3zlph",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "bigo.sg",
    "name": "Bigo",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "abv.bg",
    "name": "ABV",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "islandluck.com",
    "name": "Islandluck",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "xboxlive.com",
    "name": "Xboxlive",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "klix.ba",
    "name": "Klix",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "cisco.com",
    "name": "Cisco",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "my2n.com",
    "name": "My2n",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "lenovo.com",
    "name": "Lenovo",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "mywellness.com",
    "name": "Mywellness",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "icloud-content.com",
    "name": "Icloud Content",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "crestron.io",
    "name": "Crestron",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "2miners.com",
    "name": "2miners",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "mts.by",
    "name": "MTS",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "kufar.by",
    "name": "Kufar",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "dzen.ru",
    "name": "Dzen",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "ya.ru",
    "name": "YA",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "zerkalo.io",
    "name": "Zerkalo",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "buzzoola.com",
    "name": "Buzzoola",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "kwai.net",
    "name": "Kwai",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "globo.com",
    "name": "Globo",
    "rating": "C",
    "score": 63
  },
  {
    "domain": "kwai-pro.com",
    "name": "Kwai PRO",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "kwaipros.com",
    "name": "Kwaipros",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "uol.com.br",
    "name": "UOL",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "nubank.com.br",
    "name": "Nubank",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "iq.com",
    "name": "iQIYI",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "firefox.com",
    "name": "Firefox",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "mediatek.com",
    "name": "Mediatek",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "matrix.org",
    "name": "Matrix",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "bilibili.com",
    "name": "Bilibili",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "vivo.com.cn",
    "name": "Vivo",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "tencent-cloud.net",
    "name": "Tencent Cloud",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "douyinliving.com",
    "name": "Douyinliving",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "amap.com",
    "name": "Amap",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "douyinvod.com",
    "name": "Douyinvod",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "dbankcloud.cn",
    "name": "Dbankcloud",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "amemv.com",
    "name": "Amemv",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "zijieapi.com",
    "name": "Zijieapi",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "sogou.com",
    "name": "Sogou",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "meituan.net",
    "name": "Meituan",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "douyinpic.com",
    "name": "Douyinpic",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "ixigua.com",
    "name": "Ixigua",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "pstatp.com",
    "name": "Pstatp",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "qpic.cn",
    "name": "Qpic",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "heytapmobi.com",
    "name": "Heytapmobi",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "hdslb.com",
    "name": "Hdslb",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "snssdk.com",
    "name": "Snssdk",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "the-best-airport.com",
    "name": "THE Best Airport",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "bilivideo.com",
    "name": "Bilivideo",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "hicloud.com",
    "name": "Hicloud",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "quark.cn",
    "name": "Quark",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "jd.com",
    "name": "JD",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "etoote.com",
    "name": "Etoote",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "xiaohongshu.com",
    "name": "Xiaohongshu",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "tencent.com",
    "name": "Tencent",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "qlogo.cn",
    "name": "Qlogo",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "bcebos.com",
    "name": "Bcebos",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "mi.com",
    "name": "MI",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "live-voip.com",
    "name": "Live Voip",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "video-voip.com",
    "name": "Video Voip",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "wps.cn",
    "name": "WPS",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "biliapi.net",
    "name": "Biliapi",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "ecombdapi.com",
    "name": "Ecombdapi",
    "rating": "C",
    "score": 62
  },
  {
    "domain": "pinduoduo.com",
    "name": "Pinduoduo",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "uc.cn",
    "name": "UC",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "umeng.com",
    "name": "Umeng",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "inkuai.com",
    "name": "Inkuai",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "ndcpp.com",
    "name": "Ndcpp",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "bytedance.com",
    "name": "Bytedance",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "alipay.com",
    "name": "Alipay",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "bdurl.net",
    "name": "Bdurl",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "aliyun.com",
    "name": "Aliyun",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "sandai.net",
    "name": "Sandai",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "dianping.com",
    "name": "Dianping",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "gifshow.com",
    "name": "Gifshow",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "ksapisrv.com",
    "name": "Ksapisrv",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "xunlei.com",
    "name": "Xunlei",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "kuiniuca.com",
    "name": "Kuiniuca",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "doh.pub",
    "name": "DOH",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "begmedia.com",
    "name": "Begmedia",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "teamviewer.com",
    "name": "Teamviewer",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "westernunion.com",
    "name": "Westernunion",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "sporty-tech.net",
    "name": "Sporty Tech",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "jagnoans.com",
    "name": "Jagnoans",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "got-to-be.net",
    "name": "GOT TO BE",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "amplitude.com",
    "name": "Amplitude",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "nzherald.co.nz",
    "name": "Nzherald",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "gumgum.com",
    "name": "Gumgum",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "withgoogle.com",
    "name": "Withgoogle",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "ropro.io",
    "name": "Ropro",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "unrulymedia.com",
    "name": "Unrulymedia",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "adsrvr.org",
    "name": "Adsrvr",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "indexww.com",
    "name": "Indexww",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "stickyadstv.com",
    "name": "Stickyadstv",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "wattpad.com",
    "name": "Wattpad",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "mangageko.com",
    "name": "Mangageko",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "grpc.stream",
    "name": "Grpc",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "quran.app",
    "name": "Quran",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "consentframework.com",
    "name": "Consentframework",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "privacy-center.org",
    "name": "Privacy Center",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "beyla.site",
    "name": "Beyla",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "rqmob.com",
    "name": "Rqmob",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "pullcm.com",
    "name": "Pullcm",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "efatura.cv",
    "name": "Efatura",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "go.cr",
    "name": "GO",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "mikrotik.com",
    "name": "Mikrotik",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "mozaws.net",
    "name": "Mozaws",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "eclastio.com",
    "name": "Eclastio",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "eset.com",
    "name": "Eset",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "github.com",
    "name": "Github",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "seznam.cz",
    "name": "Seznam",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "sdn.cz",
    "name": "SDN",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "im.cz",
    "name": "IM",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "novinky.cz",
    "name": "Novinky",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "mapy.cz",
    "name": "Mapy",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "szn.cz",
    "name": "SZN",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "cra.cz",
    "name": "CRA",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "seznamzpravy.cz",
    "name": "Seznamzpravy",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "adscale.de",
    "name": "Adscale",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "sonnen.de",
    "name": "Sonnen",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "web.de",
    "name": "WEB",
    "rating": "C",
    "score": 61
  },
  {
    "domain": "nflxso.net",
    "name": "Nflxso",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "tp-link.com",
    "name": "TP Link",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "truecaller.com",
    "name": "Truecaller",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "flurry.com",
    "name": "Flurry",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "sharethis.com",
    "name": "Sharethis",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "loves.com",
    "name": "Loves",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "cookiebot.com",
    "name": "Cookiebot",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "dr.dk",
    "name": "DR",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "tv2.dk",
    "name": "Tv2",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "adnami.io",
    "name": "Adnami",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "omtrdc.net",
    "name": "Omtrdc",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "roku.com",
    "name": "Roku",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "partystar.cloud",
    "name": "Partystar",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "snackvideo.in",
    "name": "Snackvideo",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "goldenquran.org",
    "name": "Goldenquran",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "bbci.co.uk",
    "name": "Bbci",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "imo.im",
    "name": "IMO",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "fb.com",
    "name": "FB",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "imoim.app",
    "name": "Imoim",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "mncsv.com",
    "name": "Mncsv",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "sensic.net",
    "name": "Sensic",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "richaudience.com",
    "name": "Richaudience",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "delfi.ee",
    "name": "Delfi",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "cxense.com",
    "name": "Cxense",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "cb7lvj2ab.link",
    "name": "Cb7lvj2ab",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "fsapi.com",
    "name": "Fsapi",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "sanoma-sndp.fi",
    "name": "Sanoma Sndp",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "pbstck.com",
    "name": "Pbstck",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "meethue.com",
    "name": "Meethue",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "soundcloud.com",
    "name": "Soundcloud",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "nel.goog",
    "name": "NEL",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "inner-active.mobi",
    "name": "Inner Active",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "bbc.co.uk",
    "name": "BBC",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "acint.net",
    "name": "Acint",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "ge.movie",
    "name": "GE",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "opendsp.ru",
    "name": "Opendsp",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "magsrv.com",
    "name": "Magsrv",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "twilio.com",
    "name": "Twilio",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "acsechocaptiveportal.com",
    "name": "Acsechocaptiveportal",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "visualstudio.com",
    "name": "Visualstudio",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "msport.com",
    "name": "Msport",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "giphy.com",
    "name": "Giphy",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "orange.fr",
    "name": "Orange",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "vfsglobal.com",
    "name": "Vfsglobal",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "cookielaw.org",
    "name": "Cookielaw",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "recaptcha.net",
    "name": "Recaptcha",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "glomex.com",
    "name": "Glomex",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "orangeclickmedia.com",
    "name": "Orangeclickmedia",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "adman.gr",
    "name": "Adman",
    "rating": "C",
    "score": 60
  },
  {
    "domain": "wyzecam.com",
    "name": "Wyzecam",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "tabletcaptiveportal.com",
    "name": "Tabletcaptiveportal",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "index.hr",
    "name": "Index",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "connectad.io",
    "name": "Connectad",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "linker.hr",
    "name": "Linker",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "upscore.com",
    "name": "Upscore",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "midas-network.com",
    "name": "Midas Network",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "supersonicads.com",
    "name": "Supersonicads",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "shopeemobile.com",
    "name": "Shopeemobile",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "susercontent.com",
    "name": "Susercontent",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "tokopedia.com",
    "name": "Tokopedia",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "mgid.com",
    "name": "Mgid",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "typekit.net",
    "name": "Typekit",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "jiocinema.com",
    "name": "Jiocinema",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "voot.com",
    "name": "Voot",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "clevertap-prod.com",
    "name": "Clevertap Prod",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "paytm.com",
    "name": "Paytm",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "yektanet.com",
    "name": "Yektanet",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "iranlms.ir",
    "name": "Iranlms",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "bidmachine.io",
    "name": "Bidmachine",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "trademore.com",
    "name": "Trademore",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "eitaa.ir",
    "name": "Eitaa",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "aparat.com",
    "name": "Aparat",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "ea.com",
    "name": "EA",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "sanjagh.com",
    "name": "Sanjagh",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "najva.com",
    "name": "Najva",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "promizer.com",
    "name": "Promizer",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "capellare.com",
    "name": "Capellare",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "bale.ai",
    "name": "Bale",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "everestop.io",
    "name": "Everestop",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "webengage.com",
    "name": "Webengage",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "tabanweb.com",
    "name": "Tabanweb",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "megaan.homes",
    "name": "Megaan",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "maviss.cfd",
    "name": "Maviss",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "maxin.beauty",
    "name": "Maxin",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "morvaner.com",
    "name": "Morvaner",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "inna.cfd",
    "name": "Inna",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "maymay.boats",
    "name": "Maymay",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "melisa.lol",
    "name": "Melisa",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "michaeli.sbs",
    "name": "Michaeli",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "irene.lat",
    "name": "Irene",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "fyber.com",
    "name": "Fyber",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "lissa.beauty",
    "name": "Lissa",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "meroiy.icu",
    "name": "Meroiy",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "leee.motorcycles",
    "name": "Leee",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "yalla.games",
    "name": "Yalla",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "chartbeat.net",
    "name": "Chartbeat",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "k5a.io",
    "name": "K5a",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "piano.io",
    "name": "Piano",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "airserve.net",
    "name": "Airserve",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "ynet.co.il",
    "name": "Ynet",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "dxmdp.com",
    "name": "Dxmdp",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "4dex.io",
    "name": "4dex",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "spot.im",
    "name": "Spot",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "iubenda.com",
    "name": "Iubenda",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "sophosxl.net",
    "name": "Sophosxl",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "wac-msedge.net",
    "name": "WAC Msedge",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "centrastage.net",
    "name": "Centrastage",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "webex.com",
    "name": "Webex",
    "rating": "C",
    "score": 59
  },
  {
    "domain": "sophos.com",
    "name": "Sophos",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "wbx2.com",
    "name": "Wbx2",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "b-msedge.net",
    "name": "B Msedge",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "yahooapis.jp",
    "name": "Yahooapis",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "ne.jp",
    "name": "NE",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "im-apps.net",
    "name": "IM Apps",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "line-apps.com",
    "name": "Line Apps",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "rakuten.co.jp",
    "name": "Rakuten",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "line.me",
    "name": "Line",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "naver.jp",
    "name": "Naver",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "nintendo.net",
    "name": "Nintendo",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "socdm.com",
    "name": "Socdm",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "adingo.jp",
    "name": "Adingo",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "kaspi.kz",
    "name": "Kaspi",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "2gis.com",
    "name": "2gis",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "yadro.ru",
    "name": "Yadro",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "sportpesa.com",
    "name": "Sportpesa",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "urban-vpn.com",
    "name": "Urban VPN",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "reasonsecurity.com",
    "name": "Reasonsecurity",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "featuregates.org",
    "name": "Featuregates",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "mwbsys.com",
    "name": "Mwbsys",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "myqnapcloud.io",
    "name": "Myqnapcloud",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "plex.tv",
    "name": "Plex",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "notion.so",
    "name": "Notion",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "beeper.com",
    "name": "Beeper",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "anz.com",
    "name": "ANZ",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "messenger.com",
    "name": "Messenger",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "hellomini.xyz",
    "name": "Hellomini",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "g0-g3t-msg.com",
    "name": "G0 G3t MSG",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "yieldmo.com",
    "name": "Yieldmo",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "a-mo.net",
    "name": "A MO",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "dotomi.com",
    "name": "Dotomi",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "newsblur.com",
    "name": "Newsblur",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "abc.net.au",
    "name": "ABC",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "filepursuit.com",
    "name": "Filepursuit",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "yodo1.com",
    "name": "Yodo1",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "bingapis.com",
    "name": "Bingapis",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "hubspot.com",
    "name": "Hubspot",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "nytimes.com",
    "name": "Nytimes",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "heuristi.ca",
    "name": "Heuristi",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "kakao.com",
    "name": "Kakao",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "daum.net",
    "name": "Daum",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "ahnlab.com",
    "name": "Ahnlab",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "kgslb.com",
    "name": "Kgslb",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "nexon.com",
    "name": "Nexon",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "nheos.com",
    "name": "Nheos",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "navercorp.com",
    "name": "Navercorp",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "coupang.com",
    "name": "Coupang",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "pinklander.com",
    "name": "Pinklander",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "afreecatv.com",
    "name": "Afreecatv",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "fmkorea.com",
    "name": "Fmkorea",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "toss.im",
    "name": "Toss",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "dcinside.com",
    "name": "Dcinside",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "riotgames.com",
    "name": "Riotgames",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "tynt.com",
    "name": "Tynt",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "appier.net",
    "name": "Appier",
    "rating": "C",
    "score": 58
  },
  {
    "domain": "toast.com",
    "name": "Toast",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "s-onetag.com",
    "name": "S Onetag",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "edgesuite.net",
    "name": "Edgesuite",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "kaavefali.com",
    "name": "Kaavefali",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "transsion-os.com",
    "name": "Transsion OS",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "more.buzz",
    "name": "More",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "boomplaymusic.com",
    "name": "Boomplaymusic",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "adobedtm.com",
    "name": "Adobedtm",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "tawk.to",
    "name": "Tawk",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "pusher.com",
    "name": "Pusher",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "inbox.lv",
    "name": "Inbox",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "dishanywhere.com",
    "name": "Dishanywhere",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "fortinet.net",
    "name": "Fortinet",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "surfsharkstatus.com",
    "name": "Surfsharkstatus",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "dashlane.com",
    "name": "Dashlane",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "hotmail.com",
    "name": "Hotmail",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "dailymotion.com",
    "name": "Dailymotion",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "verisign.com",
    "name": "Verisign",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "admixer.net",
    "name": "Admixer",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "sentinelone.net",
    "name": "Sentinelone",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "onlymonster.ai",
    "name": "Onlymonster",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "pendo.io",
    "name": "Pendo",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "onlyfans.com",
    "name": "Onlyfans",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "appboy.com",
    "name": "Appboy",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "moengage.com",
    "name": "Moengage",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "tlivepush.com",
    "name": "Tlivepush",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "myqcloud.com",
    "name": "Myqcloud",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "toponadss.com",
    "name": "Toponadss",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "bagankeyboard.com",
    "name": "Bagankeyboard",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "amplreq.com",
    "name": "Amplreq",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "baganads.com",
    "name": "Baganads",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "mytel.com.mm",
    "name": "Mytel",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "cust-service.com",
    "name": "Cust Service",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "kbzpay.com",
    "name": "Kbzpay",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "contentexchange.me",
    "name": "Contentexchange",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "khanbank.com",
    "name": "Khanbank",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "joinhoney.com",
    "name": "Joinhoney",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "ezmob.com",
    "name": "Ezmob",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "eu.org",
    "name": "EU",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "desync.com",
    "name": "Desync",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "dler.org",
    "name": "Dler",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "stealth.si",
    "name": "Stealth",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "i2p.rocks",
    "name": "I2p",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "internetwarriors.net",
    "name": "Internetwarriors",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "explodie.org",
    "name": "Explodie",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "opentrackr.org",
    "name": "Opentrackr",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "harry.lu",
    "name": "Harry",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "rarbg.me",
    "name": "Rarbg",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "tiny-vps.com",
    "name": "Tiny VPS",
    "rating": "C",
    "score": 57
  },
  {
    "domain": "tailscale.com",
    "name": "Tailscale",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "godaddy.com",
    "name": "Godaddy",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "openbittorrent.com",
    "name": "Openbittorrent",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "opentor.org",
    "name": "Opentor",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "iptvprivateserver.ru",
    "name": "Iptvprivateserver",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "agip.gob.ar",
    "name": "GOB",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "samsungelectronics.com",
    "name": "Samsungelectronics",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "ipvanish.com",
    "name": "Ipvanish",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "onlineradiobox.com",
    "name": "Onlineradiobox",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "me.com",
    "name": "ME",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "amazonvideo.com",
    "name": "Amazonvideo",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "swannsecurity.com",
    "name": "Swannsecurity",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "grafana.com",
    "name": "Grafana",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "loc.gov",
    "name": "LOC",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "mobdro.sc",
    "name": "Mobdro",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "mookie1.com",
    "name": "Mookie1",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "nrk.no",
    "name": "NRK",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "hermes.com",
    "name": "Hermes",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "elfhosted.com",
    "name": "Elfhosted",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "cornfused.org",
    "name": "Cornfused",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "smaato.net",
    "name": "Smaato",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "bhaskar.com",
    "name": "Bhaskar",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "telegraph.co.uk",
    "name": "Telegraph",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "ultimatestremioaddons.club",
    "name": "Ultimatestremioaddons",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "6sc.co",
    "name": "6sc",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "webshare.io",
    "name": "Webshare",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "abracadabra.ag",
    "name": "Abracadabra",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "wordpress.com",
    "name": "Wordpress",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "luckyblock.cc",
    "name": "Luckyblock",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "leboncoin.fr",
    "name": "Leboncoin",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "goskope.com",
    "name": "Goskope",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "l-frii.com",
    "name": "L Frii",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "flat-ads.com",
    "name": "Flat ADS",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "opayweb.com",
    "name": "Opayweb",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "vg.no",
    "name": "VG",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "schibsted.io",
    "name": "Schibsted",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "finn.no",
    "name": "Finn",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "api.no",
    "name": "API",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "medietall.no",
    "name": "Medietall",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "vgc.no",
    "name": "VGC",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "tns-cs.net",
    "name": "TNS CS",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "omitech.site",
    "name": "Omitech",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "hamropatro.com",
    "name": "Hamropatro",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "msauth.net",
    "name": "Msauth",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "paloaltonetworks.com",
    "name": "Paloaltonetworks",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "westpac.com.au",
    "name": "Westpac",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "foxsports.com.au",
    "name": "Foxsports",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "server-cpanel.com",
    "name": "Server Cpanel",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "netsuite.com",
    "name": "Netsuite",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "dell.com",
    "name": "Dell",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "poki.com",
    "name": "Poki",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "1drv.com",
    "name": "1drv",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "workforceready.com.au",
    "name": "Workforceready",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "azure-devices.net",
    "name": "Azure Devices",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "sophosupd.com",
    "name": "Sophosupd",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "likeimo.site",
    "name": "Likeimo",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "daraz.pk",
    "name": "Daraz",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "forter.com",
    "name": "Forter",
    "rating": "C",
    "score": 56
  },
  {
    "domain": "mynt.xyz",
    "name": "Mynt",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hp.com",
    "name": "HP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "onet.pl",
    "name": "Onet",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wp.pl",
    "name": "WP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pushpushgo.com",
    "name": "Pushpushgo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "allegro.pl",
    "name": "Allegro",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pandora.com",
    "name": "Pandora",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sapo.pt",
    "name": "Sapo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sc-prod.net",
    "name": "SC Prod",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xhamster.com",
    "name": "Xhamster",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "my.com",
    "name": "MY",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mradx.net",
    "name": "Mradx",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "avito.ru",
    "name": "Avito",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tinkoff.ru",
    "name": "Tinkoff",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sberbank.ru",
    "name": "Sberbank",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ozon.ru",
    "name": "Ozon",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tns-counter.ru",
    "name": "TNS Counter",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "plmlm.com",
    "name": "Plmlm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "musiclike.top",
    "name": "Musiclike",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "snaptube.app",
    "name": "Snaptube",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "accuweather.com",
    "name": "Accuweather",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rethinkad.com",
    "name": "Rethinkad",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "lookep.com",
    "name": "Lookep",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "al-hiwar.com",
    "name": "AL Hiwar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wave.com",
    "name": "Wave",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "webrootcloudav.com",
    "name": "Webrootcloudav",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "msvdn.net",
    "name": "Msvdn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "konami.net",
    "name": "Konami",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ibytedtos.com",
    "name": "Ibytedtos",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rtp.pt",
    "name": "RTP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "blogger.com",
    "name": "Blogger",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "gravatar.com",
    "name": "Gravatar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samsungosp.com",
    "name": "Samsungosp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "scopely.io",
    "name": "Scopely",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zoznam.sk",
    "name": "Zoznam",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "svt.se",
    "name": "SVT",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "a2d.tv",
    "name": "A2d",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "applvn.com",
    "name": "Applvn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sgsnssdk.com",
    "name": "Sgsnssdk",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "anticheatexpert.com",
    "name": "Anticheatexpert",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "letspartystar.com",
    "name": "Letspartystar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pullcf.com",
    "name": "Pullcf",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "somon.tj",
    "name": "Somon",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vigo.software",
    "name": "Vigo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "linkmessenger.me",
    "name": "Linkmessenger",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nicehash.com",
    "name": "Nicehash",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adblockplus.org",
    "name": "Adblockplus",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "fwupd.org",
    "name": "Fwupd",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ntp-servers.net",
    "name": "NTP Servers",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "e-msedge.net",
    "name": "E Msedge",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cityline.com",
    "name": "Cityline",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "fontawesome.com",
    "name": "Fontawesome",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "revopush.com",
    "name": "Revopush",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "githubusercontent.com",
    "name": "Githubusercontent",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "lesta.ru",
    "name": "Lesta",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "meraki.com",
    "name": "Meraki",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "affinity.net",
    "name": "Affinity",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "omnitagjs.com",
    "name": "Omnitagjs",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pvp.net",
    "name": "PVP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "gaterelay.com",
    "name": "Gaterelay",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wxrly.top",
    "name": "Wxrly",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hinet.net",
    "name": "Hinet",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adtelligent.com",
    "name": "Adtelligent",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "idealmedia.io",
    "name": "Idealmedia",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ukr.net",
    "name": "UKR",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "firebaseio.com",
    "name": "Firebaseio",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vivint.ai",
    "name": "Vivint",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "moatads.com",
    "name": "Moatads",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "uzum.uz",
    "name": "Uzum",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ebay.com",
    "name": "Ebay",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vidoomy.com",
    "name": "Vidoomy",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zalo.me",
    "name": "Zalo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zdn.vn",
    "name": "ZDN",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zadn.vn",
    "name": "Zadn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zaloapp.com",
    "name": "Zaloapp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zing.vn",
    "name": "Zing",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "coccoc.com",
    "name": "Coccoc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "garena.com",
    "name": "Garena",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "threatseeker.com",
    "name": "Threatseeker",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xero.com",
    "name": "Xero",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "coralogix.us",
    "name": "Coralogix",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "domain.name",
    "name": "Domain",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "loina.wf",
    "name": "Loina",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "remotepc.com",
    "name": "Remotepc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "emsisoft.com",
    "name": "Emsisoft",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "idealpos.com.au",
    "name": "Idealpos",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "platinumai.net",
    "name": "Platinumai",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vipre.com",
    "name": "Vipre",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ttapis.com",
    "name": "Ttapis",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tango.me",
    "name": "Tango",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "partying.sg",
    "name": "Partying",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adsco.re",
    "name": "Adsco",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "partyother.com",
    "name": "Partyother",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bidr.io",
    "name": "Bidr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "azurefd.net",
    "name": "Azurefd",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "armdb.org",
    "name": "Armdb",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "amusnet.io",
    "name": "Amusnet",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "crazyegg.com",
    "name": "Crazyegg",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "farnell.com",
    "name": "Farnell",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "duckduckgo.com",
    "name": "Duckduckgo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "walmartimages.com",
    "name": "Walmartimages",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "textbookrush.com",
    "name": "Textbookrush",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bldrdoc.gov",
    "name": "Bldrdoc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "braze.com",
    "name": "Braze",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hpeprint.com",
    "name": "Hpeprint",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dexscreener.com",
    "name": "Dexscreener",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "canalplustech.pro",
    "name": "Canalplustech",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wix.com",
    "name": "WIX",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "steamcommunity.com",
    "name": "Steamcommunity",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samsungcloudsolution.net",
    "name": "Samsungcloudsolution",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "revenuecat.com",
    "name": "Revenuecat",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sil.org",
    "name": "SIL",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "grindr.mobi",
    "name": "Grindr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wootly.ch",
    "name": "Wootly",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bbrdbr.com",
    "name": "Bbrdbr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "alibaba.com",
    "name": "Alibaba",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "duolingo.com",
    "name": "Duolingo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dropboxusercontent.com",
    "name": "Dropboxusercontent",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sbixby.com",
    "name": "Sbixby",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sm.cn",
    "name": "SM",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cardinalcommerce.com",
    "name": "Cardinalcommerce",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ovh.net",
    "name": "OVH",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "unmsapp.com",
    "name": "Unmsapp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "anime-sama.me",
    "name": "Anime Sama",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "krxd.net",
    "name": "Krxd",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "kobotoolbox.org",
    "name": "Kobotoolbox",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "weixin.download",
    "name": "Weixin",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aniview.com",
    "name": "Aniview",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "etecsa.cu",
    "name": "Etecsa",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "disqus.com",
    "name": "Disqus",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "windscribe.com",
    "name": "Windscribe",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "fritz.box",
    "name": "Fritz",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "globalinterpark.com",
    "name": "Globalinterpark",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nist.gov",
    "name": "Nist",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "branch.io",
    "name": "Branch",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ushareit.com",
    "name": "Ushareit",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tv.fo",
    "name": "TV",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aws.dev",
    "name": "AWS",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "queue-it.net",
    "name": "Queue IT",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tkt.ge",
    "name": "TKT",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sky.com",
    "name": "SKY",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nssvc.net",
    "name": "Nssvc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "besoccer.com",
    "name": "Besoccer",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hihonorcloud.com",
    "name": "Hihonorcloud",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ajax.systems",
    "name": "Ajax",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sofascore.com",
    "name": "Sofascore",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "split.io",
    "name": "Split",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "strpst.com",
    "name": "Strpst",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jocelyn.beauty",
    "name": "Jocelyn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jolie.pics",
    "name": "Jolie",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "marary.skin",
    "name": "Marary",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "kotzting.com",
    "name": "Kotzting",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "marjorie.pics",
    "name": "Marjorie",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adwised.com",
    "name": "Adwised",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "laurren.icu",
    "name": "Laurren",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mediaad.org",
    "name": "Mediaad",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tapsell.ir",
    "name": "Tapsell",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jader.fun",
    "name": "Jader",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "userway.org",
    "name": "Userway",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mako.co.il",
    "name": "Mako",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "i-mobile.co.jp",
    "name": "I Mobile",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "me2zengame.com",
    "name": "Me2zengame",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vidaahub.com",
    "name": "Vidaahub",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samsungmax.com",
    "name": "Samsungmax",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adcolony.com",
    "name": "Adcolony",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cinlemhed.top",
    "name": "Cinlemhed",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "habboes.top",
    "name": "Habboes",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "qnap.com",
    "name": "Qnap",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wooza.top",
    "name": "Wooza",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "msftauth.net",
    "name": "Msftauth",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "yellow-resultsbidder.com",
    "name": "Yellow Resultsbidder",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mob.com",
    "name": "MOB",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "elk.zone",
    "name": "ELK",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "youtubepi.com",
    "name": "Youtubepi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ymmobi.com",
    "name": "Ymmobi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "quillbot.com",
    "name": "Quillbot",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hitomi.la",
    "name": "Hitomi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nsoft.cloud",
    "name": "Nsoft",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "standardlesothobank.co.ls",
    "name": "Standardlesothobank",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "unpkg.com",
    "name": "Unpkg",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "opera-api2.com",
    "name": "Opera Api2",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bitmoji.com",
    "name": "Bitmoji",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "codmwest.com",
    "name": "Codmwest",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dealerinspire.com",
    "name": "Dealerinspire",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tct-rom.com",
    "name": "TCT ROM",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "agkn.com",
    "name": "Agkn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mxplay.com",
    "name": "Mxplay",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "youngjoygame.com",
    "name": "Youngjoygame",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "baganintel.com",
    "name": "Baganintel",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "looktv.mn",
    "name": "Looktv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "afferdmail.com",
    "name": "Afferdmail",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "addslice.com",
    "name": "Addslice",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "appsport.com",
    "name": "Appsport",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "springboardcourses.ie",
    "name": "Springboardcourses",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "barry.blog",
    "name": "Barry",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "enterprise.com",
    "name": "Enterprise",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "honeygain.com",
    "name": "Honeygain",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nypost.com",
    "name": "Nypost",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "therealnews.com",
    "name": "Therealnews",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "welt.de",
    "name": "Welt",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ok4.se",
    "name": "Ok4",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cedexis-test.com",
    "name": "Cedexis Test",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cbssports.cloud",
    "name": "Cbssports",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "caribsurf.net",
    "name": "Caribsurf",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wwsga.me",
    "name": "Wwsga",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bytogeticr.com",
    "name": "Bytogeticr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "britishairways.com",
    "name": "Britishairways",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bmi-system.com",
    "name": "BMI System",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "baplc.com",
    "name": "Baplc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "att.net",
    "name": "ATT",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ruckuswireless.com",
    "name": "Ruckuswireless",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samsungdm.com",
    "name": "Samsungdm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xmrig.com",
    "name": "Xmrig",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "amung.us",
    "name": "Amung",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adrs.org.cn",
    "name": "Adrs",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "video4k.cc",
    "name": "Video4k",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adobecces.com",
    "name": "Adobecces",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "heroku.com",
    "name": "Heroku",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hgtv.com",
    "name": "Hgtv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "intelegridapp.com",
    "name": "Intelegridapp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "activision.com",
    "name": "Activision",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "medscape.org",
    "name": "Medscape",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "designmynight.com",
    "name": "Designmynight",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mnaspm.com",
    "name": "Mnaspm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tf1.fr",
    "name": "Tf1",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "l-msedge.net",
    "name": "L Msedge",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "byte-app.com",
    "name": "Byte APP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "go-g3t-msg.com",
    "name": "GO G3t MSG",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "buienradar.nl",
    "name": "Buienradar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "home-assistant.io",
    "name": "Home Assistant",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xhamster1.desi",
    "name": "Xhamster1",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "uisp.com",
    "name": "Uisp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mex.com.au",
    "name": "MEX",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nrl.com",
    "name": "NRL",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "statuspage.io",
    "name": "Statuspage",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "realestate.com.au",
    "name": "Realestate",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tamanu-nauru.org",
    "name": "Tamanu Nauru",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dailymail.co.uk",
    "name": "Dailymail",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xhdepot.site",
    "name": "Xhdepot",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dtvott.com",
    "name": "Dtvott",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "lazada.com.ph",
    "name": "Lazada",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "expressapisv2.net",
    "name": "Expressapisv2",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vidplay.online",
    "name": "Vidplay",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "netgear.com",
    "name": "Netgear",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "livescore.com",
    "name": "Livescore",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "equinix.com",
    "name": "Equinix",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "united.com",
    "name": "United",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bok-sd.com",
    "name": "BOK SD",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mediaset.net",
    "name": "Mediaset",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mxcontent.net",
    "name": "Mxcontent",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "histats.com",
    "name": "Histats",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "udmserve.net",
    "name": "Udmserve",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sitescout.com",
    "name": "Sitescout",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jwplayer.com",
    "name": "Jwplayer",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bugsnag.com",
    "name": "Bugsnag",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "likeevideo.ru",
    "name": "Likeevideo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "autodesk.com",
    "name": "Autodesk",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "lefigaro.fr",
    "name": "Lefigaro",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "smi2.net",
    "name": "Smi2",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adtwister.me",
    "name": "Adtwister",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "snapcraft.io",
    "name": "Snapcraft",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "auslgics.com",
    "name": "Auslgics",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tmgrup.com.tr",
    "name": "Tmgrup",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "eyeo.com",
    "name": "Eyeo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "brightcove.com",
    "name": "Brightcove",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "arin.net",
    "name": "Arin",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "syncthing.net",
    "name": "Syncthing",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cpanel.net",
    "name": "Cpanel",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "enphaseenergy.com",
    "name": "Enphaseenergy",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "watchguard.com",
    "name": "Watchguard",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "superops.ai",
    "name": "Superops",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rustdesk.com",
    "name": "Rustdesk",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "s-msedge.net",
    "name": "S Msedge",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mndsrv.com",
    "name": "Mndsrv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aopacloud.sg",
    "name": "Aopacloud",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xcoinsystem.com",
    "name": "Xcoinsystem",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dstv.com",
    "name": "Dstv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "flashtalking.com",
    "name": "Flashtalking",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cloudapp.net",
    "name": "Cloudapp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jtvnw.net",
    "name": "Jtvnw",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "msecnd.net",
    "name": "Msecnd",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "blizzard.com",
    "name": "Blizzard",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "walmart.com",
    "name": "Walmart",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "oxylabs.io",
    "name": "Oxylabs",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "prizepicks.com",
    "name": "Prizepicks",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "v3rmillion.net",
    "name": "V3rmillion",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "europcar.com",
    "name": "Europcar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "liadm.com",
    "name": "Liadm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cricbuzz.com",
    "name": "Cricbuzz",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "my3cx.fr",
    "name": "My3cx",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mexc.com",
    "name": "Mexc",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mocortech.com",
    "name": "Mocortech",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "carriersignal.info",
    "name": "Carriersignal",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "quickconnect.to",
    "name": "Quickconnect",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "seneweb.com",
    "name": "Seneweb",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dahuap2pcloud.com",
    "name": "Dahuap2pcloud",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "visaonline.com",
    "name": "Visaonline",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "swift.com",
    "name": "Swift",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "anythinktech.com",
    "name": "Anythinktech",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rainberrytv.com",
    "name": "Rainberrytv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jw.org",
    "name": "JW",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "digitalobjects.co.nz",
    "name": "Digitalobjects",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tamanubeach.com",
    "name": "Tamanubeach",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nanoleaf.me",
    "name": "Nanoleaf",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "garenanow.com",
    "name": "Garenanow",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "kenyancupid.com",
    "name": "Kenyancupid",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hentaifox.com",
    "name": "Hentaifox",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "addthis.com",
    "name": "Addthis",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "atelierfoundiboina.com",
    "name": "Atelierfoundiboina",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bbwcupid.com",
    "name": "Bbwcupid",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bentomanga.com",
    "name": "Bentomanga",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "frscan.ws",
    "name": "Frscan",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hachirumi.com",
    "name": "Hachirumi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hwtmanga.com",
    "name": "Hwtmanga",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "iconify.design",
    "name": "Iconify",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "juicyads.com",
    "name": "Juicyads",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "leak.xxx",
    "name": "Leak",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "likemanga.io",
    "name": "Likemanga",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mangadistrict.com",
    "name": "Mangadistrict",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mangahentai.me",
    "name": "Mangahentai",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mangakio.com",
    "name": "Mangakio",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mangamaniacs.org",
    "name": "Mangamaniacs",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mangas-origines.xyz",
    "name": "Mangas Origines",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mgeko.com",
    "name": "Mgeko",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "openathens.net",
    "name": "Openathens",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "quranicaudio.com",
    "name": "Quranicaudio",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "scan-vf.net",
    "name": "Scan VF",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sddan.com",
    "name": "Sddan",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "shamtick.com",
    "name": "Shamtick",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sharmadthworld.in",
    "name": "Sharmadthworld",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "streamyard.com",
    "name": "Streamyard",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "taxismaned.top",
    "name": "Taxismaned",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "terabigyellowmotha.info",
    "name": "Terabigyellowmotha",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tri.media",
    "name": "TRI",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "undertone.com",
    "name": "Undertone",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vlereader.com",
    "name": "Vlereader",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "travelocity.com",
    "name": "Travelocity",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "callofduty.com",
    "name": "Callofduty",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "codefi.network",
    "name": "Codefi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "gos-gsp.io",
    "name": "GOS GSP",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "shrkar.com",
    "name": "Shrkar",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hellay.net",
    "name": "Hellay",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tapjoy.com",
    "name": "Tapjoy",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "indazn.com",
    "name": "Indazn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ubi.com",
    "name": "UBI",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "fughm.com",
    "name": "Fughm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "josephine.fun",
    "name": "Josephine",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "linnda.beauty",
    "name": "Linnda",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "lilliane.autos",
    "name": "Lilliane",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bronze.systems",
    "name": "Bronze",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "beeper-tools.com",
    "name": "Beeper Tools",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cell.com",
    "name": "Cell",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "iask.ai",
    "name": "Iask",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mmstat.com",
    "name": "Mmstat",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "fastly-edge.com",
    "name": "Fastly Edge",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samba.tv",
    "name": "Samba",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "intercomassets.com",
    "name": "Intercomassets",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "gfx.ms",
    "name": "GFX",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mdakky.com",
    "name": "Mdakky",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "forlumineoner.com",
    "name": "Forlumineoner",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "myvideos.club",
    "name": "Myvideos",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ohchr.org",
    "name": "Ohchr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pushflow.net",
    "name": "Pushflow",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rnz.co.nz",
    "name": "RNZ",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "chatgpt.com",
    "name": "Chatgpt",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rtbadsmylive.com",
    "name": "Rtbadsmylive",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bluekai.com",
    "name": "Bluekai",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bestcontentfee.top",
    "name": "Bestcontentfee",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wpshsdk.com",
    "name": "Wpshsdk",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "xg4ken.com",
    "name": "Xg4ken",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "webrootanywhere.com",
    "name": "Webrootanywhere",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "emb-api.com",
    "name": "EMB API",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "booking.com",
    "name": "Booking",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "statsig.com",
    "name": "Statsig",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ce.it",
    "name": "CE",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "unrealengine.com",
    "name": "Unrealengine",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cryptobrowser.site",
    "name": "Cryptobrowser",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "kargo.com",
    "name": "Kargo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "habby.mobi",
    "name": "Habby",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "ntp-fireos.com",
    "name": "NTP Fireos",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "svc.ms",
    "name": "SVC",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "seek.com.au",
    "name": "Seek",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "oracle.com",
    "name": "Oracle",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mmechocaptiveportal.com",
    "name": "Mmechocaptiveportal",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sectigo.com",
    "name": "Sectigo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aninterestinghole.xyz",
    "name": "Aninterestinghole",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vod1adly.com",
    "name": "Vod1adly",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "onetrust.com",
    "name": "Onetrust",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bit.ly",
    "name": "BIT",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "sme.sk",
    "name": "SME",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rtvslo.si",
    "name": "Rtvslo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jquery.com",
    "name": "Jquery",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "amnew.net",
    "name": "Amnew",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mndlvr.com",
    "name": "Mndlvr",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "samsungknox.com",
    "name": "Samsungknox",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "exp-tas.com",
    "name": "EXP TAS",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "swiftserve.com",
    "name": "Swiftserve",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "canal-plus.com",
    "name": "Canal Plus",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "serving-sys.com",
    "name": "Serving SYS",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "classdojo.com",
    "name": "Classdojo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aftonbladet.se",
    "name": "Aftonbladet",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "symcb.com",
    "name": "Symcb",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "docker.com",
    "name": "Docker",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "vans.com",
    "name": "Vans",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "onliner.by",
    "name": "Onliner",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "quickheal.com",
    "name": "Quickheal",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "deepl.com",
    "name": "Deepl",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "zhihu.com",
    "name": "Zhihu",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "trellix.com",
    "name": "Trellix",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tapad.com",
    "name": "Tapad",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "qualcomm.com",
    "name": "Qualcomm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "readergen.fr",
    "name": "Readergen",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "a-ads.com",
    "name": "A ADS",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "spamhaus.org",
    "name": "Spamhaus",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "steamcontent.com",
    "name": "Steamcontent",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "centrum.cz",
    "name": "Centrum",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "wzrkt.com",
    "name": "Wzrkt",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "userreport.com",
    "name": "Userreport",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "dynamicyield.com",
    "name": "Dynamicyield",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "statsigapi.net",
    "name": "Statsigapi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "instructure.com",
    "name": "Instructure",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bog.ge",
    "name": "BOG",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "gouv.fr",
    "name": "Gouv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "limericker.com",
    "name": "Limericker",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "nancie.beauty",
    "name": "Nancie",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mytimeline-news.com",
    "name": "Mytimeline News",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hulabap.top",
    "name": "Hulabap",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "researchgate.net",
    "name": "Researchgate",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hentaicams.xxx",
    "name": "Hentaicams",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pushub.net",
    "name": "Pushub",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rayjump.com",
    "name": "Rayjump",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "pushapi.online",
    "name": "Pushapi",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "cloudflarestatus.com",
    "name": "Cloudflarestatus",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "atlassian.com",
    "name": "Atlassian",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "microsoftapp.net",
    "name": "Microsoftapp",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "drmgms.com",
    "name": "Drmgms",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "amp-endpoint2.com",
    "name": "AMP Endpoint2",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aircanada.com",
    "name": "Aircanada",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "usabilla.com",
    "name": "Usabilla",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "indiatimes.com",
    "name": "Indiatimes",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "aimtell.com",
    "name": "Aimtell",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "b2w.digital",
    "name": "B2w",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bazaarvoice.com",
    "name": "Bazaarvoice",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "homecu.net",
    "name": "Homecu",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "honey.io",
    "name": "Honey",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jwpltx.com",
    "name": "Jwpltx",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "jwpsrv.com",
    "name": "Jwpsrv",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "kwaixiaodian.com",
    "name": "Kwaixiaodian",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "mexiconewsdaily.com",
    "name": "Mexiconewsdaily",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "postrelease.com",
    "name": "Postrelease",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "privacy-mgmt.com",
    "name": "Privacy Mgmt",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "shopify.com",
    "name": "Shopify",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "solvvy.com",
    "name": "Solvvy",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "the-ozone-project.com",
    "name": "THE Ozone Project",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "turn.com",
    "name": "Turn",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "contentsquare.net",
    "name": "Contentsquare",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "msg-csw-lenovo.com",
    "name": "MSG CSW Lenovo",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bromium-online.com",
    "name": "Bromium Online",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adition.com",
    "name": "Adition",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "isappcloud.com",
    "name": "Isappcloud",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "connatix.com",
    "name": "Connatix",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "spribegaming.com",
    "name": "Spribegaming",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rambler.ru",
    "name": "Rambler",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "binance.com",
    "name": "Binance",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "linkm.me",
    "name": "Linkm",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "icq.net",
    "name": "ICQ",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "rozetka.com.ua",
    "name": "Rozetka",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "slackb.com",
    "name": "Slackb",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "adobelogin.com",
    "name": "Adobelogin",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "paf.com",
    "name": "PAF",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "identrust.com",
    "name": "Identrust",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "briox.services",
    "name": "Briox",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "bannerflow.com",
    "name": "Bannerflow",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "slack-edge.com",
    "name": "Slack Edge",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "onetag-sys.com",
    "name": "Onetag SYS",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "1cloudhost.net",
    "name": "1cloudhost",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "hln.be",
    "name": "HLN",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "tunnelbear.com",
    "name": "Tunnelbear",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "victronenergy.com",
    "name": "Victronenergy",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "snigelweb.com",
    "name": "Snigelweb",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "firewalla.com",
    "name": "Firewalla",
    "rating": "C",
    "score": 55
  },
  {
    "domain": "weibo.com",
    "name": "Weibo",
    "rating": "C",
    "score": 55
  }
];

const COLOR_BY_GRADE = {
  A: "#1a7f37",
  B: "#2ea44f",
  C: "#c69000",
  D: "#d65a00",
  E: "#b42318",
  DEFAULT: "#6b7280",
};

const DOMAIN_ALIASES = [
  {
    domain: "azure.com",
    hosts: ["azure.microsoft.com"],
    contains: ["azure.com", "azure.microsoft.com", "/azure"],
    titlePattern: /(^|[^a-z0-9])azure([^a-z0-9]|$)/i,
  },
];

export function normalizeHostname(value = "") {
  const raw = String(value || "").trim().toLowerCase();
  if (!raw) return "";

  try {
    const withProtocol = /^https?:\/\//i.test(raw) ? raw : `https://${raw}`;
    const parsed = new URL(withProtocol);
    return parsed.hostname.replace(/^www\./, "");
  } catch {
    return raw
      .replace(/^https?:\/\//i, "")
      .split(/[/?#]/)[0]
      .replace(/^www\./, "");
  }
}

function getReportHostname(report = {}) {
  const fromHost = normalizeHostname(report.host);
  if (fromHost) return fromHost;

  const fromUrl = normalizeHostname(report.url);
  if (fromUrl) return fromUrl;

  const fromPolicy = normalizeHostname(report.policyUrl);
  if (fromPolicy) return fromPolicy;

  return "";
}

function isSameDomainOrSubdomain(hostname, serviceDomain) {
  if (!hostname || !serviceDomain) return false;
  return hostname === serviceDomain || hostname.endsWith(`.${serviceDomain}`);
}

function getAliasDomain(report = {}, hostname = "") {
  const host = normalizeHostname(hostname);
  const url = String(report?.url || "").toLowerCase();
  const policyUrl = String(report?.policyUrl || "").toLowerCase();
  const pageTitle = String(report?.pageTitle || "").toLowerCase();
  const rawHost = String(report?.host || "").toLowerCase();
  const combined = `${rawHost} ${host} ${url} ${policyUrl} ${pageTitle}`;

  for (const alias of DOMAIN_ALIASES) {
    const hostMatches = (alias.hosts || []).some((aliasHost) => isSameDomainOrSubdomain(host, aliasHost));
    const contentMatches = (alias.contains || []).some((needle) => combined.includes(needle));
    const titleMatches = alias.titlePattern ? alias.titlePattern.test(pageTitle) : false;
    if (hostMatches || contentMatches || titleMatches) return alias.domain;
  }

  return "";
}

function slugify(value = "") {
  return String(value || "")
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
}

export function buildCatalogServiceSlugFromService(service = {}) {
  if (!service?.name || !service?.domain) return "";
  return slugify(`${service.name}-${service.domain}`);
}

export function colorFromCatalogGrade(grade) {
  return COLOR_BY_GRADE[grade] || COLOR_BY_GRADE.DEFAULT;
}

export function findCatalogServiceForReport(report = {}) {
  const hostname = getReportHostname(report);
  const aliasDomain = getAliasDomain(report, hostname);

  if (aliasDomain) {
    const aliasMatch = CATALOG_SCORE_SERVICES.find((service) => normalizeHostname(service.domain) === aliasDomain);
    if (aliasMatch) return aliasMatch;
  }

  if (!hostname) return null;

  // Se ordena por longitud para que un dominio más específico gane si existe.
  const sortedBySpecificDomain = [...CATALOG_SCORE_SERVICES].sort((a, b) => {
    return normalizeHostname(b.domain).length - normalizeHostname(a.domain).length;
  });

  return sortedBySpecificDomain.find((service) => {
    return isSameDomainOrSubdomain(hostname, normalizeHostname(service.domain));
  }) || null;
}

export function applyCatalogScore(scored = {}, report = {}) {
  const catalogService = findCatalogServiceForReport(report);
  if (!catalogService) {
    return { ...scored, catalogMatch: null, scoreSource: "live-analysis" };
  }

  return {
    ...scored,
    score: Number(catalogService.score),
    grade: catalogService.rating,
    colorHex: colorFromCatalogGrade(catalogService.rating),
    catalogMatch: {
      name: catalogService.name,
      domain: catalogService.domain,
      rating: catalogService.rating,
      score: Number(catalogService.score),
      slug: buildCatalogServiceSlugFromService(catalogService),
    },
    scoreSource: "frontend-catalog",
  };
}
