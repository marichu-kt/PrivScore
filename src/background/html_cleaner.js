...
// Convierte HTML -> texto “usable” (sin scripts/menus) para analizar políticas.
// Diseñado para funcionar en MV3 service worker (sin DOM real).

function decodeHtmlEntities(str) {
  // Decode básico sin depender de DOMParser
  // (suficiente para políticas: &amp; &lt; &gt; &quot; &#39; &nbsp;)
  return (str || "")
    .replace(/&nbsp;/gi, " ")
    .replace(/&amp;/gi, "&")
    .replace(/&lt;/gi, "<")
...
function removeSections(html) {
  ...
  // Quitar navegación/menus típicos (ojo: a veces el footer tiene links útiles,
...
function addLineBreaks(html) {
  ...
  // Convertir tags de bloque a saltos de línea para no mezclar frases
  const blockTags = [
...
function tryExtractMainLike(html) {
  // Intento “ligero” de quedarse con main/article si existe.
  ...
  // A veces el contenido está en <div id="content"> o similar: intentamos 2 heurísticas comunes
...
export function htmlToCleanText(html) {
  ...
  // 3) meter saltos de línea en bloques
  s = addLineBreaks(s);
